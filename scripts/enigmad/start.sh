...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/enigma.XXXXXXXXX")
chmod 777 "$TMP_DIR"
echo "Using temporary dir $TMP_DIR"
ENIGMAD_LOGFILE="$TMP_DIR/enigmad.log"
REST_SERVER_LOGFILE="$TMP_DIR/rest-server.log"

# This starts up enigmad
docker volume rm -f enigmad_data
docker run --rm \
  --name "$CONTAINER_NAME" \
  -p 1317:1317 \
  -p 26657:26657 \
