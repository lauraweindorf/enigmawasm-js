import { EnigmaWasmClient, findSequenceForSignedTx, IndexedTx, SearchTxFilter, types } from "@laura/enigmawasm/sdk";
import {
  Account,
  AccountQuery,
  AddressQuery,
...
export class EnigmaWasmConnection implements BlockchainConnection {
  // we must know prefix and tokens a priori to understand the chain
  public static async establish(
    url: string,
    addressPrefix: string,
    tokens: TokenConfiguration,
  ): Promise<EnigmaWasmConnection> {
    const enigmaWasmClient = new EnigmaWasmClient(url);
    const chainData = await this.initialize(enigmaWasmClient);
    return new EnigmaWasmConnection(enigmaWasmClient, chainData, addressPrefix, tokens);
  }

  private static async initialize(enigmaWasmClient: EnigmaWasmClient): Promise<ChainId> {
    const rawChainId = await enigmaWasmClient.getChainId();
  ...
  private readonly enigmaWasmClient: EnigmaWasmClient;
  ...
  private constructor(
    enigmaWasmClient: EnigmaWasmClient,
    chainId: ChainId,
    addressPrefix: string,
    tokens: TokenConfiguration,
  ) {
    this.enigmaWasmClient = enigmaWasmClient;
    this.chainId = chainId;
    this.codec = new EnigmaWasmCodec(addressPrefix, tokens.bankTokens, tokens.erc20Tokens);
  ...
  public async height(): Promise<number> {
    return this.enigmaWasmClient.getHeight();
  }
  ...
  /**
   * This is a replacement for the unimplemented EnigmaWasmCodec.identifier. Here we have more
  ...
  public async identifier(signed: SignedTransaction): Promise<TransactionId> {
    ...
    const id = await this.enigmaWasmClient.getIdentifier(tx);
    return id as TransactionId;
  }

  public async getAccount(query: AccountQuery): Promise<Account | undefined> {
    const address = isPubkeyQuery(query) ? pubkeyToAddress(query.pubkey, this.addressPrefix) : query.address;
    const bankAccount = await this.enigmaWasmClient.getAccount(address);
    ...
    const erc20Amounts = await Promise.all(
      this.erc20Tokens.map(
        async (erc20): Promise<Amount> => {
          ...
          const smart = await this.enigmaWasmClient.queryContractSmart(erc20.contractAddress, queryMsg);
  ...
  public async getNonce(query: AddressQuery | PubkeyQuery): Promise<Nonce> {
    const address = isPubkeyQuery(query) ? pubkeyToAddress(query.pubkey, this.addressPrefix) : query.address;
    const { accountNumber, sequence } = await this.enigmaWasmClient.getNonce(address);
  ...
  public async getBlockHeader(height: number): Promise<BlockHeader> {
    const { id, header, txs } = await this.enigmaWasmClient.getBlock(height);
    return {
      id: id as BlockId,
  ...
  public async getTx(
    ...
    const results = await this.enigmaWasmClient.searchTx({ id: id });
  ...
  public async postTx(tx: PostableBytes): Promise<PostTxResponse> {
    ...
    const { transactionHash, rawLog } = await this.enigmaWasmClient.postTx(txAsJson);
  ...
  public async searchTx({
    ...
    if (id) {
      txs = await this.enigmaWasmClient.searchTx({ id: id }, filter);
    } else if (height) {
      txs = await this.enigmaWasmClient.searchTx({ height: height }, filter);
    } else if (sentFromOrTo) {
      const pendingRequests = new Array<Promise<readonly IndexedTx[]>>();
      pendingRequests.push(this.enigmaWasmClient.searchTx({ sentFromOrTo: sentFromOrTo }, filter));
      for (const contract of this.erc20Tokens.map(token => token.contractAddress)) {
        ...
        pendingRequests.push(this.enigmaWasmClient.searchTx({ tags: searchBySender }, filter));
        pendingRequests.push(this.enigmaWasmClient.searchTx({ tags: searchByRecipient }, filter));
      }
  ...
  private async parseAndPopulateTxResponseSigned(
    ...
    const { accountNumber, sequence: currentSequence } = await this.enigmaWasmClient.getNonce(senderAddress);
