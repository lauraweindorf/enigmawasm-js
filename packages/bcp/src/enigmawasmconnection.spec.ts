import { decodeSignature } from "@laura/enigmawasm/sdk";
import {
  Account,
  Address,
  Algorithm,
...
const faucet = {
  ...
  path: HdPaths.enigma(0),
  pubkey: {
    algo: Algorithm.Secp256k1,
    data: fromBase64("A08EGB7ro1ORuFhjOnZcSgwYlpe0DSFjVNUIkNNQxwKQ") as PubkeyBytes,
  },
  address: "enigma1pkptre7fdkl6gfrzlesjjvhxhlc3r4gmmk8rs6" as Address,
};

describe("EnigmaWasmConnection", () => {
  const scrt = "SCRT" as TokenTicker;
  const isa = "ISA" as TokenTicker;
  const httpUrl = "http://localhost:1317";
  const defaultChainId = "enigma:testing" as ChainId;
  const defaultEmptyAddress = "enigma1h806c7khnvmjlywdrkdgk2vrayy2mmvf9rxk2r" as Address;
  const defaultRecipient = "enigma1t70qnpr0az8tf7py83m4ue5y89w58lkjmx0yq2" as Address;
  ...
  const unusedAccount = {
    ...
    address: "enigma1cjsxept9rkggzxztslae9ndgpdyt2408lk850u" as Address,
  };

  // this is for enigmad blockchain
  ...
  const scrtConfig: TokenConfiguration = {
    bankTokens: [
      {
        fractionalDigits: 6,
        name: "Scrt",
  ...
  describe("establish", () => {
    it("can connect to Enigma via http", async () => {
      pendingWithoutEnigmad();
      const connection = await EnigmaWasmConnection.establish(httpUrl, defaultAddressPrefix, defaultConfig);
  ...
  describe("getToken", () => {
    it("displays a given token", async () => {
      pendingWithoutEnigmad();
      const connection = await EnigmaWasmConnection.establish(httpUrl, defaultAddressPrefix, defaultConfig);
      const token = await connection.getToken("SCRT" as TokenTicker);
  ...
  describe("identifier", () => {
    it("calculates tx hash from PostableBytes", async () => {
      pendingWithoutEnigmad();
      const connection = await EnigmaWasmConnection.establish(httpUrl, defaultAddressPrefix, atomConfig);
